...
class DiseaseDetectionService:
    ...
    def initialize(self):
        """Initialize service"""
        # Models are loaded via model_loader in app startup (lifespan)
        # copy disease_classes for quick access
    ...
    async def predict_disease(
        ...
        try:
            ...
            if not disease_classes:
                raise ValueError(f"No disease classes metadata found for crop '{crop_type}'")
            ...
            # Get top prediction
            disease_idx = int(np.argmax(preds[0]))
            confidence = float(preds[0][disease_idx])
            ...
            top_3_indices = list(np.argsort(preds[0])[-3:][::-1])
            top_3_predictions = [
                {
            ...
            result = {
                ...
                'all_predictions': {
                    (disease_classes[i] if i < len(disease_classes) else f"idx_{i}"): float(preds[0][i])
